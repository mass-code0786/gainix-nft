...
async function buildState(db: DbClient) {
  ...
  const state: NftSimState = {
    users: users.map<UserRecord>((item) => ({
      ...
      referralCode: item.referralCode ?? item.walletAddress,
    ...
    nft_trades: nftTrades.map<NftTradeRecord>((item) => ({
      ...
      tradingWalletUsed: (item as { tradingWalletUsed?: number }).tradingWalletUsed ?? item.buyPrice,
      withdrawalWalletUsed: (item as { withdrawalWalletUsed?: number }).withdrawalWalletUsed ?? 0,
      totalBuyAmount: (item as { totalBuyAmount?: number }).totalBuyAmount ?? item.buyPrice,
    ...
    bot_subscriptions: botSubscriptions.map<BotSubscriptionRecord>((item) => ({
      ...
      activatedByAdmin: item.activatedByAdmin,
  ...
  if (state.nft_trades.length > 0) {
    await tx.nFTTrade.createMany({
      data: state.nft_trades.map((item) => ({
        ...
        userId: item.userId,
        buyPrice: item.buyPrice,
        tradingWalletUsed: item.tradingWalletUsed,
        withdrawalWalletUsed: item.withdrawalWalletUsed,
        totalBuyAmount: item.totalBuyAmount || item.buyPrice,
        sellPrice: item.sellPrice,
        profit: item.profit,
